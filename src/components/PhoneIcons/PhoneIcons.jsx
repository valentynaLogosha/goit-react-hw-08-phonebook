...
const PhoneIcons = () => {
  useEffect(() => {
    const createPhone = () => {
      ...
      const minSize = 1;
      const maxSize = 20;
      const minDelay = 2000; // Збільшена затримка
      const maxDelay = 5000; // Збільшена затримка
      ...
      const updatePhone = () => {
        ...
        let delay = Math.random() * (maxDelay - minDelay) + minDelay; // Збільшена затримка
    ...
    let numPhone = 2; //  кількість іконок
